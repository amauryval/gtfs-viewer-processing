...
def test_data_processing_full_data_thresh_overwrite_table(credentials, session_db):
    ...
    MovingPoints.set_session(next(session_db))
    assert MovingPoints.schemas() == {"gtfs_data"}
    assert MovingPoints.infos().rows_count == 407

    date = datetime.datetime.strptime('2007-01-01 08:02:40', date_format)
    query = MovingPoints.filter_by_date_area(date, "fake", [-180.0, -89.0, 180.0, 89.0])
...
def test_data_processing_full_data_calendar_dates(credentials, session_db):
    ...
    date = datetime.datetime.strptime('2007-06-04 08:00:20', date_format)
    query = MovingPoints.filter_by_date_area(date, "fake", [-180.0, -89.0, 180.0, 89.0])
    assert query.count() == 1
...
def test_data_processing_full_data_threshold(credentials, session_db):
    ...
    date = datetime.datetime.strptime('2007-01-01 08:01:40', date_format)
    query = MovingPoints.filter_by_date_area(date, "fake", [-180.0, -89.0, 180.0, 89.0])
